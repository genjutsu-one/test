(function () {
    ...
    var res = await fetch("https://api.onlysq.ru/ai/openai/chat/completions", {  // ← ИСПРАВЛЕНО
        method: "POST",
        headers: { 
    ...
    if (!res.ok) {
        var errorText = await res.text().catch(() => "Unknown error");
    ...
    return (content || "").trim() || "(пустой ответ от ИИ)";
    }
